...
# Cycles Management Script for HEFIN Decentralized Platform (Updated for dfx cycles)
set -e

echo "💰 HEFIN Cycles Management"
echo "============================="
...
# MODIFIED: Updated list of HEFIN Canisters
CANISTERS=("hefin_user_data" "hefin_claims_logic" "hefin_ai_fraud" "hefin_financial_ledger" "hefin_integration_api" "hefin_ui_canister")
...
for canister in "${CANISTERS[@]}"; do
    ...
    if [ -n "$CANISTER_ID" ]; then
        ...
        if [ -n "$STATUS" ]; then
            # Extract cycles balance from the status output
            ...
            if [ "$CYCLES" != "0" ]; then
                ...
                if [ $CYCLES -lt 1000000000000 ]; then
                    # NOTE: Low cycles are especially critical for the hefin_ai_fraud canister due to high computation costs.
                    echo -e "${YELLOW}  ⚠️  Low cycles warning! Top up recommended.${NC}"
                fi
            else
...
if [ "$NETWORK" = "ic" ]; then
    echo ""
    echo -e "${BLUE}💳 Cycles Top-up Options (Focus on ensuring AI/Logic Canisters are funded):${NC}"
